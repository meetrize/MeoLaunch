...
- (instancetype)init {
    ...
    if (self) {
        ...
        _maxEntries = 48;
        _iconPointSize = 32.0;
        _queue = dispatch_queue_create("com.meetrice.meolaunch.taskbar.iconcache", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)loadIconForPath:(NSString *)path onLoaded:(void (^)(NSString *path, NSImage *image))onLoaded {
    ...
    CGFloat pointSize = self.iconPointSize > 0 ? self.iconPointSize : 32.0;
